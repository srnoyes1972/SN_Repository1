...
def get_pie_chart(site):
    ...
    if site == 'ALL':
        ...
        df2 = df.groupby(['Launch Site', 'class']).size().reset_index(name='class count')
        fig = px.pie(df2, values='class count', names='class', title=f"Total Success Launches For Site {site}")
        return fig
...
def get_scatter_chart(site, slrange):
    low, high = slrange
